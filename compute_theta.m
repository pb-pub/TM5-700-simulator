...
elseif t > (tacc + tab) && t <= (tacc + tab + 2*tacc)
    start_theta = compute_theta(tacc + tab, tacc, tdab, tdbc, thA, thB, thC, ts);
    delta_t = t - (tacc + tab);
    theta = start_theta + thpab * delta_t + 1/2 * thppb * delta_t^2;
elseif t > (tacc + tab + 2*tacc) && t <= (tacc + tab + 2*tacc + tbc) 
    start_theta = compute_theta(tacc + tab + 2*tacc, tacc, tdab, tdbc, thA, thB, thC, ts);
    delta_t = t - (tacc + tab + 2*tacc);
    theta = start_theta + thpbc * delta_t;
elseif t > (tacc + tab + 2*tacc + tbc) && t <= (tdab + tdbc)
    start_theta = compute_theta(tacc + tab + 2*tacc + tbc, tacc, tdab, tdbc, thA, thB, thC, ts);
    delta_t = t - (tacc + tab + 2*tacc + tbc);
